...
class ClientRepository:
    ...
    async def list(self, limit: int = 20, offset: int = 0) -> list[Client]:
        result = await self._s.execute(
            select(Client)
            .where(Client.tenant_id == get_current_tenant_id(), Client.deleted_at.is_(None))
            .limit(limit)
            .offset(offset)
        )
...
class ProjectRepository:
    ...
    async def list(
        ...
        q = select(Project).where(
            Project.tenant_id == get_current_tenant_id(), Project.deleted_at.is_(None)
        )
        if status:
    ...
    async def get(self, milestone_id: uuid.UUID) -> Milestone | None:
        result = await self._s.execute(
            select(Milestone).where(
                ...
                Milestone.tenant_id == get_current_tenant_id(),
...
class TaskRepository:
    ...
    async def create(
        self, project_id: uuid.UUID, reporter_user_id: uuid.UUID, **kwargs: object
    ) -> Task:
        obj = Task(
    ...
    async def get(self, comment_id: uuid.UUID) -> Comment | None:
        result = await self._s.execute(
            select(Comment).where(
                ...
                Comment.tenant_id == get_current_tenant_id(),
    ...
    async def get(self, attachment_id: uuid.UUID) -> Attachment | None:
        result = await self._s.execute(
            select(Attachment).where(
                ...
                Attachment.tenant_id == get_current_tenant_id(),
