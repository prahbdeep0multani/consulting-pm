...
class UserRepository:
    ...
    async def create(self, tenant_id: uuid.UUID, email: str, password_hash: str, full_name: str) -> User:
        user = User(tenant_id=tenant_id, email=email, password_hash=password_hash, full_name=full_name)
    ...
    async def list_by_tenant(self, tenant_id: uuid.UUID, limit: int = 20, offset: int = 0) -> list[User]:
        result = await self._session.execute(
            select(User)
            .options(selectinload(User.user_roles).selectinload(UserRole.role))
    ...
    async def assign_role(self, user: User, role: Role, assigned_by: uuid.UUID | None = None) -> None:
...
class RoleRepository:
    ...
    async def create(self, tenant_id: uuid.UUID, name: str, permissions: list[str], is_system: bool = False) -> Role:
