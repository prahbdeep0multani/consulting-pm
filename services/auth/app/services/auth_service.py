...
from shared.core.exceptions import AuthenticationError, NotFoundError
...
class AuthService:
    ...
    async def refresh(
        ...
        self._jwt.decode_refresh_token(refresh_token)
        ...
        access, refresh = await self._issue_token_pair(
            user, ip_address, user_agent, family=stored.family
        )
        await self._session.commit()
    ...
    async def _issue_token_pair(
        ...
        new_family = uuid.UUID(
            self._jwt.decode_refresh_token(raw_refresh).family or str(uuid.uuid4())
        )
        expires_at = datetime.now(UTC) + timedelta(days=self._jwt._refresh_expire)
